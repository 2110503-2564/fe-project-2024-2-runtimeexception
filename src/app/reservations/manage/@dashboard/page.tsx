...
async function DashboardPage() {
  ...
  const addCar = async (addCarForm: FormData) => {
    "use server";
    const model = addCarForm.get("model") as string;
    const description = addCarForm.get("desc") as string;
    const picture = addCarForm.get("picture") as string;
    const seats = parseInt(addCarForm.get("seats") as string);
    const doors = parseInt(addCarForm.get("doors") as string);
    const largebags = parseInt(addCarForm.get("largebags") as string);
    const smallbags = parseInt(addCarForm.get("smallbags") as string);
    const automatic = addCarForm.get("automatic") === "on";
    const dayRate = addCarForm.get("dayRate") as string;
    try {
      await dbConnect();
      await Car.create({
        model: model,
